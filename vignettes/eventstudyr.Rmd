...
We will use the `df_sample_static` and `df_sample_dynamic` datasets to demonstrate the basic functionality of `eventstudyr`. These sample data are from the [replication archive](https://data.nber.org/data-appendix/w29170/) for Freyaldenhoven et al. (2021). The documentation can be accessed using `?df_sample_static` and `?df_sample_dynamic`.
 

```{r Data preview df_sample_static}

dim(df_sample_static)
head(df_sample_static)
```

```{r Data preview df_sample_dynamic}
...
```{r Optional vars example - Show code, eval = F}
results <- EventStudy(estimator = "OLS",
                      data = df_sample_static,
                      outcomevar = "y_static",
                      policyvar = "z",
...
```
```{r Optional vars example - Run code, eval = T, echo = F}
results <- EventStudy(estimator = "OLS",
                      data = df_sample_static,
                      outcomevar = "y_static",
                      policyvar = "z",
                     ...
                     anticipation_effects_normalization = TRUE)
```
```{r Optional vars example - Show Results 1, echo=TRUE, eval=TRUE}
    results[1]
```
<details>
  <summary>Click for `results[2]`</summary>
